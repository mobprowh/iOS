...
class TinyPixView: UIView {
    ...
    private func calculateGridForSize(_ size: CGSize) {
        ...
        gridRect = CGRect(x: (size.width - space)/2, y: (size.height - space)/2, width: space, height: space)
    }
    ...
    private func drawBlockAt(row: Int, column: Int) {
        ...
        let blockFrame = CGRect(x: startX, y: startY, width: blockSize.width, height: blockSize.height)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        selectedBlockIndex = touchedGridIndexFromTouches(touches: touches)
        toggleSelectedBlock()
    }
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        let touched = touchedGridIndexFromTouches(touches: touches)
