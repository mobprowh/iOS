...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let labelFrame = CGRect(x: bounds.origin.x, y: bounds.midY - 50, width: bounds.size.width, height: 100)
        ...
        segmentedControl.addTarget(self, action: #selector(selectionChanged), for: UIControlEvents.valueChanged)
    ...
    func rotateLabelDown() {
        UIView.animate(withDuration: 1.0, animations: {
            ...
            completion: {(Bool) -> Void in self.rotateLabelUp()}
        )
    }
    func rotateLabelUp() {
        UIView.animate(withDuration: 1.0, animations: {
