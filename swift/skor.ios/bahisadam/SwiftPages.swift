...
open class SwiftPages: UIView {
    private lazy var __once: () = {
            ...
            self.animatedBar = UIView(frame: CGRect(x: 0, y: self.topBarHeight - self.animatedBarHeight + 1, width: (self.containerView.frame.size.width / CGFloat(pageCount)), height: self.animatedBarHeight))
            ...
            if self.barShadow {
                self.shadowView = UIView(frame: CGRect(x: 0, y: self.topBarHeight, width: self.containerView.frame.size.width, height: 6))
                self.shadowViewGradient.frame = self.shadowView.bounds
                self.shadowViewGradient.colors = [UIColor(red: 120/255, green: 120/255, blue: 120/255, alpha: 0.5).cgColor, UIColor.clear.cgColor]
    ...
    // Item size variables
    fileprivate var topBarHeight: CGFloat = 50
    fileprivate var animatedBarHeight: CGFloat = 5
    ...
    fileprivate var buttonsTextFontAndSize = UIFont(name: "HelveticaNeue-Bold", size: 15)!
    ...
    open func alignSubviews() {
        ...
        animatedBar.frame.size = CGSize(width: (containerView.frame.size.width / (CGFloat)(pageCount)), height: animatedBarHeight)
        if barShadow {
...
extension SwiftPages: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let offsetAddition = (containerView.frame.size.width / CGFloat(pageCount)) * 0.0
